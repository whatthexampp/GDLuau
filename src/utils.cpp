...
void lua_pushvariant(lua_State *L, const godot::Variant &var) {
	switch (var.get_type()) {
		...
		case godot::Variant::Type::COLOR: {
			godot::Color c = var.operator godot::Color();
			godot::Dictionary dict;
			dict["r"] = c.r;
			dict["g"] = c.g;
			dict["b"] = c.b;
			dict["a"] = c.a;
			lua_pushdictionary(L, dict);
			break;
		}
...
godot::Variant lua_tovariant(lua_State *L, int idx) {
	...
	switch (type) {
		...
		case LUA_TSTRING: {
			...
			return s ? godot::String(s) : godot::String("null");
		}
...
godot::Variant lua_toarray(lua_State *L, int idx) {
	...
	size_t length = lua_objlen(L, abs_idx);
	godot::Array array;
...
bool luaL_isarray(lua_State *L, int idx) {
	...
	size_t len = lua_objlen(L, abs_idx);
	if (len == 0) {
		lua_pushvalue(L, abs_idx);
		lua_pushnil(L);
		if (lua_next(L, -2) != 0) {
...
godot::Object *lua_toobject(lua_State *L, int idx) {
	...
	if (p_nodewrapped == NULL)
		return NULL;
	if (!luaL_hasmetatable(L, idx, "object"))
		return NULL;
...
godot::LuauVM *lua_getnode(lua_State *L) {
	lua_getfield(L, LUA_REGISTRYINDEX, GDLUAU_REGISTRY_NODE_KEY);
